...
public class MonitorController extends BaseController {
	...
	public List<MonitorTopic> getTopicList(@RequestParam("cluster") String clusterId) {
		...
		List<ClusterInfo> clusterInfos = "-1".equalsIgnoreCase(clusterId) ? clusterService.getTotalData()
				: Collections.singletonList(clusterService.selectById(Long.parseUnsignedLong(clusterId)));
	...
	@ResponseBody
	public RestResponse getCollectionList(@PathParam(value = Constants.JsonObject.TYPE) String type,
			@RequestParam("cluster") String clusterId) {
		List<MonitorTopic> monitorTopics = new ArrayList<>();
		UserInfo user = getCurrentUser();
	...
	@ResponseBody
	public RestResponse collections(@PathParam(value = Constants.JsonObject.NAME) String name,
	...
	public RestResponse topicGroup(@RequestBody Map<String, String> queryMap) {
		...
		if (config.isRemoteQueryEnable() && remoteHostsMap.containsKey(info.getLocation().toLowerCase())) {
			String url = request.getScheme() + Constants.Symbol.COLON + Constants.Symbol.DOUBLE_SLASH + request.getServerName()
					+ Constants.Symbol.COLON + request.getServerPort() + Constants.Symbol.SLASH + Constants.KeyStr.REMOTE
					+ request.getServletPath().replaceAll(Constants.Symbol.SLASH + Constants.KeyStr.MONITOR, Constants.Symbol.EMPTY_STR);
	...
	public RestResponse topicMetric(@RequestBody Map<String, String> queryMap) {
		...
		if (config.isRemoteQueryEnable() && remoteHostsMap.containsKey(info.getLocation().toLowerCase())) {
			String url = request.getScheme() + Constants.Symbol.COLON + Constants.Symbol.DOUBLE_SLASH + request.getServerName()
					+ Constants.Symbol.COLON + request.getServerPort() + request.getServletPath().replaceAll(
							Constants.Symbol.SLASH + Constants.KeyStr.MONITOR, Constants.Symbol.SLASH + Constants.KeyStr.REMOTE);
			JSONArray data = restService.queryRemoteQuery(url, queryMap);
			return SUCCESS_DATA(data);
		} else {
