...
public class MonitorController extends BaseController {
	...
	public RestResponse topicGroup(@RequestBody Map<String, String> queryMap) {
		...
		if (config.isRemoteQueryEnable() && remoteHostsMap.containsKey(info.getLocation().toLowerCase())) {
			String url = request.getScheme() + Constants.Symbol.COLON + Constants.Symbol.DOUBLE_SLASH
					+ request.getServerName() + Constants.Symbol.COLON + request.getServerPort()
					+ Constants.Symbol.SLASH + Constants.KeyStr.REMOTE + request.getServletPath()
	...
	public RestResponse topicMetric(@RequestBody Map<String, String> queryMap) {
		...
		if (config.isRemoteQueryEnable() && remoteHostsMap.containsKey(info.getLocation().toLowerCase())) {
			String url = request.getScheme() + Constants.Symbol.COLON + Constants.Symbol.DOUBLE_SLASH
					+ request.getServerName() + Constants.Symbol.COLON + request.getServerPort()
					+ request.getServletPath().replaceAll(Constants.Symbol.SLASH + Constants.KeyStr.MONITOR,
							Constants.Symbol.SLASH + Constants.KeyStr.REMOTE);
			JSONArray data = restService.queryRemoteQuery(url, queryMap);
			return SUCCESS_DATA(data);
		} else {
